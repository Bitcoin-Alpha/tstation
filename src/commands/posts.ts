...
export function postsCommand(globals: () => GlobalOpts): Command {
  ...
  posts
    ...
    .addOption(new Option("--featured <bool>", "filter by featured flag").choices(["true", "false"]))
    .addOption(
      new Option("--read-status <status>", "filter by read status (requires auth)").choices(["all", "read", "unread"]),
    )
    ...
    .action(async (o: Record<string, string>) => {
      ...
      if (o.featured) query.featured = o.featured as ListQuery["featured"];
      if (o.readStatus) query.read_status = o.readStatus as ListQuery["read_status"];
